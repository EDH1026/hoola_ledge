...
### 9.2 게임 목록 — 구간 내 인별 점수 합계

- `/games`의 기간 필터(년/월/일)를 적용했을 때, 현재는 전체 점수 합계만 보여준다.
- 요구사항: **해당 구간 내 참가자별 점수 합계**를 함께 표시한다. 구간 내 각 참가자가 딴 점수, 잃은 점수, 순점수를 순위와 함께 보여준다(순점수 내림차순 정렬 권장).

### 9.3 이스포츠 스타일 통계/대시보드 확장

모임의 재미를 높이는 것이 목적이다. LoL 같은 이스포츠 서비스의 전적/랭킹 화면에서 쓰는 표현 방식을 참고한다.

#### 9.3.1 대시보드

- **핫 플레이어 / 콜드 플레이어**: 최근 폼이 통산 성적 대비 얼마나 좋은지/나쁜지를 기준으로 상승세·하락세 플레이어를 각각 강조한다. 최근 구간의 기준(최근 N경기 또는 최근 N일)과 비교 방식은 구현 시 합리적으로 정하되, 표본이 너무 적을 때(예: 최근 구간 경기 수가 극히 적음) 과장된 결과가 나오지 않도록 최소 표본 조건을 둔다.
- **최근 폼 표시**: 각 참가자의 최근 5경기 결과를 W/L 뱃지 시퀀스로 표시(승패가 갈리지 않은 참석은 제외하거나 별도 표기).
- **현재 연승/연패 스트릭**: 진행 중인 연승·연패를 뱃지로 표시.
- **오늘의 요약**: 오늘 진행된 게임 수, 오늘의 최다 승자 등 간단한 요약.

#### 9.3.2 통계 화면

- **티어/랭크 뱃지**: 순포인트 또는 승률을 기준으로 참가자에게 등급(브론즈~챌린저 형태)을 부여해 리더보드에 뱃지로 표시. 기준은 구현 시 정하고 화면에 간단히 설명한다.
- **상대 전적 매트릭스**: 전체 참가자 × 전체 참가자 그리드로 상대 전적을 한눈에 보여준다(순점수 기준 색상 강도 표현 권장).
- **천적 / 밥**: 각 참가자별로 가장 많이 진 상대와 가장 많이 이긴 상대를 표시(기존 head-to-head 데이터 활용).
- **종목별 성적**: 참가자 × 종목별 승률·순포인트(이스포츠의 "챔피언 숙련도"에 해당). 종목별 최강자를 별도로 강조해도 좋다.
- **기록실(Records)**: 최장 연승, 최장 연패, 단일 게임 최고 점수, 하루 최다 승리, 최다 참석(개근왕) 등 명예의 전당 성격의 기록 모음.
- 위 항목들에는 가능한 범위에서 기존 기간·종목 필터가 적용되어야 한다. 필터 적용이 의미상 맞지 않는 항목(예: 통산 기록실)은 통산 기준임을 명시한다.

#### 9.3.3 구현 지침

- 새 통계는 전부 기존 `src/lib/stats.ts`에 순수 함수로 추가해, 서버/클라이언트 양쪽에서 재사용 가능하게 한다(현재 구조와 동일).
- 소프트 삭제된 게임 제외, 레거시 레코드의 기본값 처리(`points ?? 1` 등) 규칙을 모든 신규 집계에도 동일하게 적용한다.
- 데이터가 적을 때(참가자 2~3명, 게임 몇 판)도 화면이 깨지거나 오해를 부르지 않도록 빈 상태(empty state)를 처리한다.

### 9.4 성공 기준

- 휴대폰 브라우저에서 게임 결과 입력이 문제없이 완료된다.
- 기부 기록 후 잔액이 "주는 사람 감소 / 받는 사람 증가" 방향으로 반영된다.
