...
- "기부"는 예전의 "탕감"을 일반화한 개념으로, 잔액 반영 공식은 정산과 동일합니다:
  주는 사람(`fromId`) 잔액 `+= amount`, 받는 사람(`toId`) 잔액 `-= amount`.
  계산된 특정 채무-채권 거래에 묶이지 않고 임의의 두 참가자 사이에서 자유로운
  금액으로 기록할 수 있습니다(원래 갚아야 할 금액보다 많이 줘도 됨).
